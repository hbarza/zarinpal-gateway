...
class Codnitive_ZarinpalGateway_Model_System_Config_Backend_Cert extends Mage_Core_Model_Config_Data
{
    ...
    protected function _beforeSave()
    {
        ...
        if ($value) {
            try {
                ...
                if ($condition) {
                    $err = Mage::helper('zarinpalgateway')->__('%s is not registered.', 
                            Mage::helper('zarinpalgateway')->__(Codnitive_ZarinpalGateway_Model_Config::EXTENSION_NAME));
                ...
                try {
                    ...
                    curl_setopt($ch, CURLOPT_URL, Mage::helper('zarinpalgateway')->getConUrl());
                ...
                if (false == $data) {
                    $err = Mage::helper('zarinpalgateway')->__('%s save error. Please try again later.', 
                            Mage::helper('zarinpalgateway')->__(Codnitive_ZarinpalGateway_Model_Config::EXTENSION_NAME));
                    Mage::throwException($err);
                }
                if ('1' !== $data) {
                    $err = Mage::helper('zarinpalgateway')->__('Serial Number')
                        . Mage::helper('zarinpalgateway')->__(' or registration information for')
                        . Mage::helper('zarinpalgateway')->__(' %s', Mage::helper('zarinpalgateway')->__(Codnitive_ZarinpalGateway_Model_Config::EXTENSION_NAME))
                        . Mage::helper('zarinpalgateway')->__(' is not valid. Please check them or %s', Mage::helper('zarinpalgateway')->__('contact us.'));
                    Mage::throwException($err);
                }
            }
            catch (Exception $e) {
